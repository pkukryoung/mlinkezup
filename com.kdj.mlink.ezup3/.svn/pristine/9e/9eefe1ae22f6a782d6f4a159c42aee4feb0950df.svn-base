package com.kdj.mlink.ezup3.shop.dao;

public class ShopProductGmarketAdditionDto extends ShopProductAdditionDto {

	private String seq;//일련번호
	private String compno;//회사코드
	private String shopcd;//쇼핑몰코드
	private String title;//제목
	private String shopid;//쇼핑몰아이디
	private String memo;//메모
	private String selmthdcd;//형태구분
	private String value1;//필수속성정보
	private String value2;//브랜드ON카테고리선택
	private String value71;//BIZON카테고리선택
	private String value3;//소호노출범위여부
	private String value4;//소호노출카테고리
	private String value5;//소호노출스타일
	private String value6;//소호노출샤프
	private String value7;//소호노출칼라
	private String value9;//모델명적용여부
	private String value10;//모델명
	private String value11;//모델명수정여부
	private String value12;//ESM브랜드
	private String prodstatcd;//상품상태
	private String usedperiod;//중고개월수
	private String maximumqtytyp;//최대구매허용수량여부
	private String limitqty;//제한수량
	private String limittime;//제한기간
	private String optexhibition;//옵션재고전시방법
	private String value14;//옵션노출정렬여부
	private String inputtypopt;//입력형옵션
	private String value15;//추가상품전시여부
	private String salesperiod;//판매기간
	private String image;//대표이미지
	private String prodinfoarea;//상품정보입력영역
	private String prodadditional;//추가구성입력영역
	private String promotion;//광고/홍보입력영역
	private String value72;//상품정보입력영역중문
	private String value73;//상품정보입력영역영문
	private String adultgoods;//성인용품여부
	private String childprod;//어린이제품
	private String homeholdprod;//생활용품
	private String electrical;//전기용품
	private String chemistry;//생활화학/살생물제품
	private String eco;//친환경
	private String medical;//의료기기
	private String origin;//원산지선택여부
	private String multiorigin;//복수원산지여부
	private String asdtl;//A/S정보
	private String shippingpolicy;//발송정책
	private String shippingmethod;//배송방법
	private String express;//택배사
	private String value17;//방문수령여부
	private String value69;//방문수령가격할인금액
	private String value70;//방문수령사은품
	private String value18;//방문수령주소
	private String value19;//퀵서비스업체명
	private String value20;//퀵서비스연락처
	private String value21;//퀵서비스배송가능지역
	private String value22;//오늘출발여부
	private String outaddr;//출하지
	private String bundledyn;//묶음배송여부
	private String bundlemaxnmin;//묶음배송시배송비
	private String template;//묶음배송시템플릿
	private String delvcosttyp;//택배비무료여부
	private String delvcost;//배송기본배송비
	private String delvfreecost;//기본배송비2
	private String costdiscount;//배송비할인여부
	private String value24;//수량별차등수량1
	private String value25;//수량별차등수량2
	private String value26;//수량별차등수량3
	private String value27;//수량별차등수량4
	private String value28;//수량별차등수량5
	private String value29;//수량별차등배송비1
	private String value30;//수량별차등배송비2
	private String value31;//수량별차등배송비3
	private String value32;//수량별차등배송비4
	private String value33;//수량별차등배송비5
	private String prepayment;//착/선불여부
	private String retnexchangeaddr;//교환/반품주소
	private String retnexchangecost;//교환/반품배송비
	private String orderdelaytime;//주문후예상배송기간
	private String value37;//판매자관리코드
	private String value38;//판매자직접입력관리코드
	private String value39;//판매자카테고리
	private String value40;//사은품
	private String value41;//포털가격비교사이트등록여부
	private String value42;//포털가격비교사이트쿠폰적용여부
	private String value74;//해외판매여부
	private String value43;//사이트부담지원할인
	private String value44;//휴대폰가입신청서URL
	private String value45;//렌탈설치비
	private String value46;//렌탈의무사용기간
	private String value47;//렌탈등록비
	private String value48;//옵션관리코드여부
	private String value49;//기타특이사항
	private String value50;//상품명앞추가문구
	private String value51;//상품명뒷추가문구
	private String value52;//상품설명상단추가문구
	private String value53;//상품설명하단추가문구
	private String value54;//판매자부담할인여부
	private String value55;//판매자부담할인금액
	private String value56;//판매자부담할인시작일
	private String value57;//판매자부담할인종료일
	private String value58;//복수구매여부
	private String value59;//복수구매시작일
	private String value60;//복수구매종료일
	private String value61;//구매수량금액
	private String value62;//구매수량할인율
	private String value63;//구매수량상품별금액
	private String value64;//구매수량상품별할인율
	private String value75;//구매수량별무료갯수
	private String value65;//스마일캐시적립률
	private String value66;//후원쇼핑시작일
	private String value67;//후원쇼핑종료일
	private String value68;//후원쇼핑금액
	private String value76;//후원쇼핑누적적립한도액
	private String value77;//후원분야
	private String value78;//포커스등록여부
	private String value79;//포커스기간연장등록여부
	private String value80;//포커스기간연장일자
	private String value81;//포커스자동연장여부
	private String value82;//포커스무제한시종료일
	private String value83;//포커스기간시종료일
	private String value84;//포커스기간시종료일2
	private String value85;//포커스플러스등록여부
	private String value86;//포커스플러스기간연장등록여부
	private String value87;//포커스플러스기간연장일자
	private String value88;//포커스플러스자동연장여부
	private String value89;//포커스플러스무제한시종료일
	private String value90;//포커스플러스기간시종료일
	private String value91;//포커스플러스기간시종료일2
	private String useyn;//사용여부
	public String getSeq() {
		return seq;
	}
	public void setSeq(String seq) {
		this.seq = seq;
	}
	public String getCompno() {
		return compno;
	}
	public void setCompno(String compno) {
		this.compno = compno;
	}
	public String getShopcd() {
		return shopcd;
	}
	public void setShopcd(String shopcd) {
		this.shopcd = shopcd;
	}
	public String getTitle() {
		return title;
	}
	public void setTitle(String title) {
		this.title = title;
	}
	public String getShopid() {
		return shopid;
	}
	public void setShopid(String shopid) {
		this.shopid = shopid;
	}
	public String getMemo() {
		return memo;
	}
	public void setMemo(String memo) {
		this.memo = memo;
	}
	public String getSelmthdcd() {
		return selmthdcd;
	}
	public void setSelmthdcd(String selmthdcd) {
		this.selmthdcd = selmthdcd;
	}
	public String getValue1() {
		return value1;
	}
	public void setValue1(String value1) {
		this.value1 = value1;
	}
	public String getValue2() {
		return value2;
	}
	public void setValue2(String value2) {
		this.value2 = value2;
	}
	public String getValue71() {
		return value71;
	}
	public void setValue71(String value71) {
		this.value71 = value71;
	}
	public String getValue3() {
		return value3;
	}
	public void setValue3(String value3) {
		this.value3 = value3;
	}
	public String getValue4() {
		return value4;
	}
	public void setValue4(String value4) {
		this.value4 = value4;
	}
	public String getValue5() {
		return value5;
	}
	public void setValue5(String value5) {
		this.value5 = value5;
	}
	public String getValue6() {
		return value6;
	}
	public void setValue6(String value6) {
		this.value6 = value6;
	}
	public String getValue7() {
		return value7;
	}
	public void setValue7(String value7) {
		this.value7 = value7;
	}
	public String getValue9() {
		return value9;
	}
	public void setValue9(String value9) {
		this.value9 = value9;
	}
	public String getValue10() {
		return value10;
	}
	public void setValue10(String value10) {
		this.value10 = value10;
	}
	public String getValue11() {
		return value11;
	}
	public void setValue11(String value11) {
		this.value11 = value11;
	}
	public String getValue12() {
		return value12;
	}
	public void setValue12(String value12) {
		this.value12 = value12;
	}
	public String getProdstatcd() {
		return prodstatcd;
	}
	public void setProdstatcd(String prodstatcd) {
		this.prodstatcd = prodstatcd;
	}
	public String getUsedperiod() {
		return usedperiod;
	}
	public void setUsedperiod(String usedperiod) {
		this.usedperiod = usedperiod;
	}
	public String getMaximumqtytyp() {
		return maximumqtytyp;
	}
	public void setMaximumqtytyp(String maximumqtytyp) {
		this.maximumqtytyp = maximumqtytyp;
	}
	public String getLimitqty() {
		return limitqty;
	}
	public void setLimitqty(String limitqty) {
		this.limitqty = limitqty;
	}
	public String getLimittime() {
		return limittime;
	}
	public void setLimittime(String limittime) {
		this.limittime = limittime;
	}
	public String getOptexhibition() {
		return optexhibition;
	}
	public void setOptexhibition(String optexhibition) {
		this.optexhibition = optexhibition;
	}
	public String getValue14() {
		return value14;
	}
	public void setValue14(String value14) {
		this.value14 = value14;
	}
	public String getInputtypopt() {
		return inputtypopt;
	}
	public void setInputtypopt(String inputtypopt) {
		this.inputtypopt = inputtypopt;
	}
	public String getValue15() {
		return value15;
	}
	public void setValue15(String value15) {
		this.value15 = value15;
	}
	public String getSalesperiod() {
		return salesperiod;
	}
	public void setSalesperiod(String salesperiod) {
		this.salesperiod = salesperiod;
	}
	public String getImage() {
		return image;
	}
	public void setImage(String image) {
		this.image = image;
	}
	public String getProdinfoarea() {
		return prodinfoarea;
	}
	public void setProdinfoarea(String prodinfoarea) {
		this.prodinfoarea = prodinfoarea;
	}
	public String getProdadditional() {
		return prodadditional;
	}
	public void setProdadditional(String prodadditional) {
		this.prodadditional = prodadditional;
	}
	public String getPromotion() {
		return promotion;
	}
	public void setPromotion(String promotion) {
		this.promotion = promotion;
	}
	public String getValue72() {
		return value72;
	}
	public void setValue72(String value72) {
		this.value72 = value72;
	}
	public String getValue73() {
		return value73;
	}
	public void setValue73(String value73) {
		this.value73 = value73;
	}
	public String getAdultgoods() {
		return adultgoods;
	}
	public void setAdultgoods(String adultgoods) {
		this.adultgoods = adultgoods;
	}
	public String getChildprod() {
		return childprod;
	}
	public void setChildprod(String childprod) {
		this.childprod = childprod;
	}
	public String getHomeholdprod() {
		return homeholdprod;
	}
	public void setHomeholdprod(String homeholdprod) {
		this.homeholdprod = homeholdprod;
	}
	public String getElectrical() {
		return electrical;
	}
	public void setElectrical(String electrical) {
		this.electrical = electrical;
	}
	public String getChemistry() {
		return chemistry;
	}
	public void setChemistry(String chemistry) {
		this.chemistry = chemistry;
	}
	public String getEco() {
		return eco;
	}
	public void setEco(String eco) {
		this.eco = eco;
	}
	public String getMedical() {
		return medical;
	}
	public void setMedical(String medical) {
		this.medical = medical;
	}
	public String getOrigin() {
		return origin;
	}
	public void setOrigin(String origin) {
		this.origin = origin;
	}
	public String getMultiorigin() {
		return multiorigin;
	}
	public void setMultiorigin(String multiorigin) {
		this.multiorigin = multiorigin;
	}
	public String getAsdtl() {
		return asdtl;
	}
	public void setAsdtl(String asdtl) {
		this.asdtl = asdtl;
	}
	public String getShippingpolicy() {
		return shippingpolicy;
	}
	public void setShippingpolicy(String shippingpolicy) {
		this.shippingpolicy = shippingpolicy;
	}
	public String getShippingmethod() {
		return shippingmethod;
	}
	public void setShippingmethod(String shippingmethod) {
		this.shippingmethod = shippingmethod;
	}
	public String getExpress() {
		return express;
	}
	public void setExpress(String express) {
		this.express = express;
	}
	public String getValue17() {
		return value17;
	}
	public void setValue17(String value17) {
		this.value17 = value17;
	}
	public String getValue69() {
		return value69;
	}
	public void setValue69(String value69) {
		this.value69 = value69;
	}
	public String getValue70() {
		return value70;
	}
	public void setValue70(String value70) {
		this.value70 = value70;
	}
	public String getValue18() {
		return value18;
	}
	public void setValue18(String value18) {
		this.value18 = value18;
	}
	public String getValue19() {
		return value19;
	}
	public void setValue19(String value19) {
		this.value19 = value19;
	}
	public String getValue20() {
		return value20;
	}
	public void setValue20(String value20) {
		this.value20 = value20;
	}
	public String getValue21() {
		return value21;
	}
	public void setValue21(String value21) {
		this.value21 = value21;
	}
	public String getValue22() {
		return value22;
	}
	public void setValue22(String value22) {
		this.value22 = value22;
	}
	public String getOutaddr() {
		return outaddr;
	}
	public void setOutaddr(String outaddr) {
		this.outaddr = outaddr;
	}
	public String getBundledyn() {
		return bundledyn;
	}
	public void setBundledyn(String bundledyn) {
		this.bundledyn = bundledyn;
	}
	public String getBundlemaxnmin() {
		return bundlemaxnmin;
	}
	public void setBundlemaxnmin(String bundlemaxnmin) {
		this.bundlemaxnmin = bundlemaxnmin;
	}
	public String getTemplate() {
		return template;
	}
	public void setTemplate(String template) {
		this.template = template;
	}
	public String getDelvcosttyp() {
		return delvcosttyp;
	}
	public void setDelvcosttyp(String delvcosttyp) {
		this.delvcosttyp = delvcosttyp;
	}
	public String getDelvcost() {
		return delvcost;
	}
	public void setDelvcost(String delvcost) {
		this.delvcost = delvcost;
	}
	public String getDelvfreecost() {
		return delvfreecost;
	}
	public void setDelvfreecost(String delvfreecost) {
		this.delvfreecost = delvfreecost;
	}
	public String getCostdiscount() {
		return costdiscount;
	}
	public void setCostdiscount(String costdiscount) {
		this.costdiscount = costdiscount;
	}
	public String getValue24() {
		return value24;
	}
	public void setValue24(String value24) {
		this.value24 = value24;
	}
	public String getValue25() {
		return value25;
	}
	public void setValue25(String value25) {
		this.value25 = value25;
	}
	public String getValue26() {
		return value26;
	}
	public void setValue26(String value26) {
		this.value26 = value26;
	}
	public String getValue27() {
		return value27;
	}
	public void setValue27(String value27) {
		this.value27 = value27;
	}
	public String getValue28() {
		return value28;
	}
	public void setValue28(String value28) {
		this.value28 = value28;
	}
	public String getValue29() {
		return value29;
	}
	public void setValue29(String value29) {
		this.value29 = value29;
	}
	public String getValue30() {
		return value30;
	}
	public void setValue30(String value30) {
		this.value30 = value30;
	}
	public String getValue31() {
		return value31;
	}
	public void setValue31(String value31) {
		this.value31 = value31;
	}
	public String getValue32() {
		return value32;
	}
	public void setValue32(String value32) {
		this.value32 = value32;
	}
	public String getValue33() {
		return value33;
	}
	public void setValue33(String value33) {
		this.value33 = value33;
	}
	public String getPrepayment() {
		return prepayment;
	}
	public void setPrepayment(String prepayment) {
		this.prepayment = prepayment;
	}
	public String getRetnexchangeaddr() {
		return retnexchangeaddr;
	}
	public void setRetnexchangeaddr(String retnexchangeaddr) {
		this.retnexchangeaddr = retnexchangeaddr;
	}
	public String getRetnexchangecost() {
		return retnexchangecost;
	}
	public void setRetnexchangecost(String retnexchangecost) {
		this.retnexchangecost = retnexchangecost;
	}
	public String getOrderdelaytime() {
		return orderdelaytime;
	}
	public void setOrderdelaytime(String orderdelaytime) {
		this.orderdelaytime = orderdelaytime;
	}
	public String getValue37() {
		return value37;
	}
	public void setValue37(String value37) {
		this.value37 = value37;
	}
	public String getValue38() {
		return value38;
	}
	public void setValue38(String value38) {
		this.value38 = value38;
	}
	public String getValue39() {
		return value39;
	}
	public void setValue39(String value39) {
		this.value39 = value39;
	}
	public String getValue40() {
		return value40;
	}
	public void setValue40(String value40) {
		this.value40 = value40;
	}
	public String getValue41() {
		return value41;
	}
	public void setValue41(String value41) {
		this.value41 = value41;
	}
	public String getValue42() {
		return value42;
	}
	public void setValue42(String value42) {
		this.value42 = value42;
	}
	public String getValue74() {
		return value74;
	}
	public void setValue74(String value74) {
		this.value74 = value74;
	}
	public String getValue43() {
		return value43;
	}
	public void setValue43(String value43) {
		this.value43 = value43;
	}
	public String getValue44() {
		return value44;
	}
	public void setValue44(String value44) {
		this.value44 = value44;
	}
	public String getValue45() {
		return value45;
	}
	public void setValue45(String value45) {
		this.value45 = value45;
	}
	public String getValue46() {
		return value46;
	}
	public void setValue46(String value46) {
		this.value46 = value46;
	}
	public String getValue47() {
		return value47;
	}
	public void setValue47(String value47) {
		this.value47 = value47;
	}
	public String getValue48() {
		return value48;
	}
	public void setValue48(String value48) {
		this.value48 = value48;
	}
	public String getValue49() {
		return value49;
	}
	public void setValue49(String value49) {
		this.value49 = value49;
	}
	public String getValue50() {
		return value50;
	}
	public void setValue50(String value50) {
		this.value50 = value50;
	}
	public String getValue51() {
		return value51;
	}
	public void setValue51(String value51) {
		this.value51 = value51;
	}
	public String getValue52() {
		return value52;
	}
	public void setValue52(String value52) {
		this.value52 = value52;
	}
	public String getValue53() {
		return value53;
	}
	public void setValue53(String value53) {
		this.value53 = value53;
	}
	public String getValue54() {
		return value54;
	}
	public void setValue54(String value54) {
		this.value54 = value54;
	}
	public String getValue55() {
		return value55;
	}
	public void setValue55(String value55) {
		this.value55 = value55;
	}
	public String getValue56() {
		return value56;
	}
	public void setValue56(String value56) {
		this.value56 = value56;
	}
	public String getValue57() {
		return value57;
	}
	public void setValue57(String value57) {
		this.value57 = value57;
	}
	public String getValue58() {
		return value58;
	}
	public void setValue58(String value58) {
		this.value58 = value58;
	}
	public String getValue59() {
		return value59;
	}
	public void setValue59(String value59) {
		this.value59 = value59;
	}
	public String getValue60() {
		return value60;
	}
	public void setValue60(String value60) {
		this.value60 = value60;
	}
	public String getValue61() {
		return value61;
	}
	public void setValue61(String value61) {
		this.value61 = value61;
	}
	public String getValue62() {
		return value62;
	}
	public void setValue62(String value62) {
		this.value62 = value62;
	}
	public String getValue63() {
		return value63;
	}
	public void setValue63(String value63) {
		this.value63 = value63;
	}
	public String getValue64() {
		return value64;
	}
	public void setValue64(String value64) {
		this.value64 = value64;
	}
	public String getValue75() {
		return value75;
	}
	public void setValue75(String value75) {
		this.value75 = value75;
	}
	public String getValue65() {
		return value65;
	}
	public void setValue65(String value65) {
		this.value65 = value65;
	}
	public String getValue66() {
		return value66;
	}
	public void setValue66(String value66) {
		this.value66 = value66;
	}
	public String getValue67() {
		return value67;
	}
	public void setValue67(String value67) {
		this.value67 = value67;
	}
	public String getValue68() {
		return value68;
	}
	public void setValue68(String value68) {
		this.value68 = value68;
	}
	public String getValue76() {
		return value76;
	}
	public void setValue76(String value76) {
		this.value76 = value76;
	}
	public String getValue77() {
		return value77;
	}
	public void setValue77(String value77) {
		this.value77 = value77;
	}
	public String getValue78() {
		return value78;
	}
	public void setValue78(String value78) {
		this.value78 = value78;
	}
	public String getValue79() {
		return value79;
	}
	public void setValue79(String value79) {
		this.value79 = value79;
	}
	public String getValue80() {
		return value80;
	}
	public void setValue80(String value80) {
		this.value80 = value80;
	}
	public String getValue81() {
		return value81;
	}
	public void setValue81(String value81) {
		this.value81 = value81;
	}
	public String getValue82() {
		return value82;
	}
	public void setValue82(String value82) {
		this.value82 = value82;
	}
	public String getValue83() {
		return value83;
	}
	public void setValue83(String value83) {
		this.value83 = value83;
	}
	public String getValue84() {
		return value84;
	}
	public void setValue84(String value84) {
		this.value84 = value84;
	}
	public String getValue85() {
		return value85;
	}
	public void setValue85(String value85) {
		this.value85 = value85;
	}
	public String getValue86() {
		return value86;
	}
	public void setValue86(String value86) {
		this.value86 = value86;
	}
	public String getValue87() {
		return value87;
	}
	public void setValue87(String value87) {
		this.value87 = value87;
	}
	public String getValue88() {
		return value88;
	}
	public void setValue88(String value88) {
		this.value88 = value88;
	}
	public String getValue89() {
		return value89;
	}
	public void setValue89(String value89) {
		this.value89 = value89;
	}
	public String getValue90() {
		return value90;
	}
	public void setValue90(String value90) {
		this.value90 = value90;
	}
	public String getValue91() {
		return value91;
	}
	public void setValue91(String value91) {
		this.value91 = value91;
	}
	public String getUseyn() {
		return useyn;
	}
	public void setUseyn(String useyn) {
		this.useyn = useyn;
	}

	
	
	
	
		
}
